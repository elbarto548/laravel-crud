...
@section('javascript')
    <script>
        ...
        function modalEditar(id) {
            const ruta = route("curso.edit",[id]);
            axios.get(ruta)
            .then(function(response){
    ...
    const ruta = route("curso.update", id); // Asegúrate de que la ruta esté configurada para PUT
    const formulario = document.getElementById("formulario-editar");
    const data = new FormData(formulario);
    
    // Añadir el método PUT a los datos del formulario
    data.append('_method', 'PUT'); // Laravel utiliza un campo especial para manejar métodos diferentes a GET y POST
    
    limpiarErrores("formulario-editar");
    
    axios.post(ruta, data) // Puedes seguir usando POST ya que Laravel lo interpreta correctamente con el campo _method
    .then(function(response) {
        const message = response.data.message;
        toastr.success(message);
        $('#modal-editar').modal('hide');
        search(); // Llama a tu función de búsqueda para actualizar la vista
    })
    .catch(function(error) {
        if (error.response) {
            // Entidad improcesable
        ...
        function confirmarEliminar(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                   ...
                   const ruta = route('curso.destroy', [id]);
